...
--ISNULL(A3.ImportFS, 0) ImportFS, ISNULL(A4.ImportFS, 0) Adeudo, ISNULL(A4.U_GLO_CodeMov, '') AS CodeMove
--from
...
--                                WHERE Canceled = 'N' AND U_GLO_FolioOri = @RiseId and 
--                                                                     U_GLO_PaymentType='GLSOV') 
--                                                       group by U_GLO_Auxiliar, Account 
--) A3 ON A3.U_GLO_Auxiliar = A0.EmpId and a3.Account = @Account
--left join
--(    Select sum(F0.ImportFS) ImportFS, F0.Account, F0.U_GLO_Auxiliar, U_GLO_CodeMov
--     from (
--          SELECT TOP 1 SUM(ISNULL(Debit, 0)) - SUM(ISNULL(Credit, 0)) ImportFS, Account, U_GLO_Auxiliar, t2.TaxDate, U_GLO_CodeMov
...
--                                                                  and U_GLO_PaymentType='GLSOV'
--                ) group by U_GLO_Auxiliar, Account , U_GLO_CodeMov, t2.TaxDate, U_GLO_CodeMov
--              having SUM(ISNULL(Debit, 0)) - SUM(ISNULL(Credit, 0)) >0 order by t2.TaxDate desc
--         ) F0 
--         group by F0.Account, F0.U_GLO_Auxiliar, F0.U_GLO_CodeMov
--) A4 ON A4.U_GLO_Auxiliar = A0.EmpId and a4.Account = @Account

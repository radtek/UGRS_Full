...
left join (select 
 F0.OcrCode2 Equipo,
sum(case when F3.QryGroup26='Y' and f0.Tipo = 'T' then case when f1.DocEntry is not null then f1.Quantity else f2.Quantity end else 0 end) '15W40',
sum(case when F3.QryGroup28='Y' and f0.Tipo = 'T' then case when f1.DocEntry is not null then f1.Quantity else f2.Quantity end else 0 end) Hidraulico,
sum(case when F3.QryGroup29='Y' and f0.Tipo = 'T' then case when f1.DocEntry is not null then f1.Quantity else f2.Quantity end else 0 end) SAE_40,
sum(case when F3.QryGroup30='Y' and f0.Tipo = 'T' then case when f1.DocEntry is not null then f1.Quantity else f2.Quantity end else 0 end) Transm,
sum(case when F3.QryGroup23='Y' and f0.Tipo = 'T' then case when f1.DocEntry is not null then f1.Quantity else f2.Quantity end else 0 end) Grasa
from (
       ...
       from (  ----- Universo de facturas y traslados de cada AF
              ...
              union all
                     ...
                     select 
                              a1.DocDate, A1.DocEntry, a0.Tipo, A0.OcrCode2 
                      from
                     (----- Busca los primeros DocEntrys de los AF en las solicitudes
                           select 
                                   min(t0.DocEntry) Folio, t1.OcrCode2, 'S' Tipo
                           from OWTQ T0 with (nolock)
                           inner join wtq1 t1 with (nolock) on t1.DocEntry = t0.DocEntry
                           where t1.WhsCode ='MQHEOBRA' and T0.U_MQ_Rise = @FolioOri and t1.OcrCode2 is not null and t1.Quantity>0 and t0.CANCELED='N'
                           group by t1.OcrCode2
                     ) A0
                     inner join OWTQ A1 on a1.DocEntry = A0.Folio
       ) B0
) F0
left join wtr1 F1 with (nolock) on F1.DocEntry = F0.DocEntry and f0.Tipo = 'T' and f1.OcrCode2 = f0.OcrCode2
left join wtq1 F2 with (nolock) on F2.DocEntry = F0.DocEntry and f0.Tipo = 'S' and f2.OcrCode2 = f0.OcrCode2

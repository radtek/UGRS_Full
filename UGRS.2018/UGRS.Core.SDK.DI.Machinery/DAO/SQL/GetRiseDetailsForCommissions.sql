...
(select max(case when a1.U_Supervisor=a0.IDEmp then 'S' else 'O' end) Puesto, a0.Tipo,a0.IDEmp EmpId, sum(a0.Horas) Horas from (
select 'S' Tipo, s1.U_Supervisor IDEmp, sum(s1.u_hrfeet) Horas
 from [@UG_MQ_RIHR] s1 with (nolock) where s1.u_idrise= @RiseId group by s1.U_Supervisor
 union all 
 select 'S' Tipo, s2.U_Supervisor IDEmp, sum(s2.U_Hrs) Horas from [@UG_MQ_RIHT] s2 with (nolock) 
 where s2.u_idrise= @RiseId group by s2.U_Supervisor, s2.U_Hrs
 union all
select 'O' Tipo, s1.U_Operator IDEmp, sum(s1.u_hrfeet) Horas
 from [@UG_MQ_RIHR] s1 with (nolock) where s1.u_idrise= @RiseId group by s1.U_Operator
 union all 
 select 'O' Tipo, s2.U_Operator IDEmp, sum(s2.U_Hrs) Horas from [@UG_MQ_RIHT] s2 with (nolock) 
 where s2.u_idrise= @RiseId group by s2.U_Operator, s2.U_Hrs) A0
 left join [@UG_TBL_MQ_RISE] A1 on a1.U_IdRise= @RiseId
 group by a0.Tipo,a0.IDEmp) A0
 inner join OHEM A1 on a0.EmpId = a1.empID
inner join 
(

	select T0.DocEntry, Max(T0.DocNum) DocNum, Max(T0.DocDate) DocDate, DATEDIFF(day, Max(T0.DocDueDate), GetDate()) as Days, Max(T1.OcrCode) OcrCode, max(case when t0.DocTotalFC > 0 then t0.DocTotalFC else t0.DocTotal end) - max(t0.PaidToDate) as Balance from OINV T0 
	...
	--where   T0.CardCode = '{CardCode}' and DocStatus = 'O' and T0.CANCELED = 'N' --T2.DocEntry is null and T3.DocEntry is null and

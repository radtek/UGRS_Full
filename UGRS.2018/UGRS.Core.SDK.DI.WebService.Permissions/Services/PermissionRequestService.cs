...
using UGRS.Core.SDK.DI.WebServicePermissions.DAO;
using UGRS.Core.SDK.DI.WebServicePermissions.Tables;
using UGRS.Core.Services;

namespace UGRS.Core.SDK.DI.WebServicePermissions.Services
{
    ...
    public class PermissionRequestService
    {
        ...
        public int Add(PermissionRequest pObjRecord)
        {
            ...
            if (!mObjPermissionsDAO.ExistRequest(pObjRecord.RequestId))
            {
                ...
                {
                    lStrPrefix = mObjPermissionsDAO.GetPrefix("PE_SERIE_EX");
                }
                else
                {
                    lStrPrefix = mObjPermissionsDAO.GetPrefix("PE_SERIE_NOEXP");
                }
                ...
                else
                {
                    ...
                    {
                        LogService.WriteError("[PermissionRequest NO CREATE]");
                    }
        ...
        public int TestCreateSalerOrder(string pStrRequestId)
        {
            ...
            if(mObjPermissionsDAO.IsRequestPreparedToCreateSaleOrder(pStrRequestId) && !mObjPermissionsDAO.ExistsSaleOrder(pStrRequestId)) {
                lIntResult = mObjPermissionsDAO.CreateSaleOrder(pStrRequestId);
            }

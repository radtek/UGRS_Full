SELECT
	...
	((
		...
		FROM [@UG_PE_WS_PORE] PO
			INNER JOIN [@UG_PE_ENRY] EN ON PO.U_PortId = EN.U_IdSAGARHPA
		WHERE 
	...
	((
		SELECT 
			TOP 1 EN.Code 
		FROM [@UG_PE_WS_PORE] PO
			INNER JOIN [@UG_PE_DERE] EN ON PO.U_PortId = EN.U_IdSAGARHPA
		WHERE 
			PO.U_RequestId = PE.U_RequestId AND  
			PO.U_PortType = 2
	...
	(
		CASE WHEN
			...
			ELSE
			(
				CONCAT(DE.U_State, ', ', DE.U_City, ', ', DE.U_Location)
			)
		END
	...
	--No tomar en cuenta la aduana 2
	--ISNULL
	--((
	--	SELECT 
	--		TOP 1 EN.Code 

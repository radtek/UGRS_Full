...
namespace UGRS.AddOn.Finances.Forms
{
    ...
    class Payments : UserFormBase
    {
        ...
        private void SBO_Application_ItemEvent(string FormUID, ref SAPbouiCOM.ItemEvent pVal, out bool BubbleEvent)
        {
                ...
                if (!pVal.BeforeAction)
                {
                    switch (pVal.EventType)
                    {
                        ...
                        case SAPbouiCOM.BoEventTypes.et_VALIDATE:
                            ...
                            {
                                this.FlushValueToSource(mMtxClientInvoices, pVal.ColUID, pVal.Row);
                                //mDtClientInvoices.SetValue("C_Difference", pVal.Row - 1, "");
                                ReloadAmounts();
                            }
                            //Update datatable when altering invoice's amount to pay and update amounts
            ...
            {
                LogUtility.WriteError(string.Format("[Payments - SBO_Application_ItemEvent] Error: {0}", ex.Message));
        ...
        private void LoadClient(string pCardCode)
        {
            ...
            try
            {
                ...
                if (!mChkDrafts.Checked)
                {
                    ...
                    foreach (InvoiceDTO lObjinvoice in lLstInvoices)
                    {
                        ...
                        mDtClientInvoices.SetValue("C_Difference", i, lObjinvoice.DocRemaining);
                        ...
                        mDtClientInvoices.SetValue("C_Used", i, "N");
                        i++;
                ...
                else
                {
                    ...
                    foreach (DraftDTO lObjDraft in lLstDrafts)
                    {
                        if (lObjDraft.OcrCode == lStrCostingCenter || lObjDraft.UserSign == lIntUserSign || (lBolHasRole && lObjDraft.U_GLO_CashRegister == "Y"))
                        {
                            ...
                            mDtClientInvoices.SetValue("C_Difference", i, lObjDraft.DocTotal);
            ...
            {
                LogUtility.WriteError(string.Format("[Payments - LoadClient] Error: {0}", lObjException.Message));
                throw lObjException;
            }
            finally
            {
        ...
        private void LoadAccountPayments(string pCardCode)
        {
            ...
            {
                LogUtility.WriteError(string.Format("[Payments - LoadAccountPayments] Error: {0}", lObjException.Message));
                throw lObjException;
            }
        }
        ...
        private void LoadChecks(string pCardCode)
        {
            ...
            {
                LogUtility.WriteError(string.Format("[Payments - LoadChecks] Error: {0}", lObjException.Message));
                throw lObjException;
            }
        }
        ...
        private void LoadCreditCards()
        {
            ...
            {
                LogUtility.WriteError(string.Format("[Payments - LoadCreditCards] Error: {0}", lObjException.Message));
                throw lObjException;
            }
        }
        ...
        private double CalculateCheckTotal()
        {
            // Reload Current
            double lDblTotal = 0;
            // Checks
            for (int i = 0; i < mDtPayChecks.Rows.Count; i++)
            {
                lDblTotal += mDtPayChecks.GetCellValue<double>("C_CheckSum", i);
            }
        ...
        private void GenerateDocuments()
        {
            try
            {
                ...
                GenerateCheckDocuments(lStrCostingCenter, ref lDblCheckTotal);
                ...
                if (!mChkDrafts.Checked)
                {
                    ...
                    for (int i = 0; i < mDtClientInvoices.Rows.Count; i++)
                    {
                        if (mDtClientInvoices.GetCellValue<string>("C_Selected", i) == "Y" && mDtClientInvoices.GetCellValue<double>("C_DocRem", i) > 0
                            && mDtClientInvoices.GetCellValue<string>("C_Used", i) == "N")
                        {
                ...
                else
                {
                    ...
                    UIApplication.ShowSuccess(string.Format("Pago generado exitosamente con el DocEntry: {0}", lStrLastDoc));

                    AddDocumentsItem(lStrLastDoc);
            ...
            {
                LogUtility.WriteError(string.Format("[Payments - GenerateDocuments] Error: {0}", lObjException.Message));
                throw lObjException;
            }
        }

        private void GenerateCheckDocuments(string pStrCostingCenter, ref double lDblCheckTotal)
        {
            List<int> lChecksToDeposit = new List<int>();
            int lIntChekDeposit = -1;
            double lDblTotal = CalculateCheckTotal();
            try
            {
                lDblCheckTotal = 0;

                //IList<InvoiceDTO> lLstInvoicesDTO = mDocumentDAO.DataTableToInvoiceDTO(mDtClientInvoices);
                ...
                for (int i = 0; i < mMtxPayChecks.RowCount; i++)
                {
                    ...
                    string lStrCahsAcct = mCheckDAO.GetCheckAcct(UIApplication.GetCompany().UserName);
                    if (string.IsNullOrEmpty(lStrCahsAcct))
                    {
                        throw new Exception(string.Format("El usuario {0} no tiene asignada una cuenta de caja en sus propiedades.", UIApplication.GetCompany().UserName));
                    }

                    // GL Account
                    if (mEdtChecksAccount.Value == "")
                    {
                        throw new Exception("No se especifico una cuenta para cheques.");
                    }
                    ...
                    if (lDteDueDate == null)
                    {
                        throw new Exception(String.Format("No se especifico fecha de vencimiento en el cheque no. {0}", i + 1));
                    }
                    if (lDteDueDate < DateTime.Now)
                    {
                        throw new Exception(String.Format("El cheque no. {0} esta vencido o vence hoy.", i + 1));
                    }
                    ...
                    if (lStrBankCode == "")
                    {
                        throw new Exception(String.Format("No se especifico banco en el cheque no. {0}", i + 1));
                    }
                    ...
                    if (lStrAccountCode == "")
                    {
                        throw new Exception(String.Format("No se especifico cuenta en el cheque no. {0}", i + 1));
                    }
                    ...
                    if (lStrCheckNum == 0)
                    {
                        throw new Exception(String.Format("No se especifico folio en cheque no. {0}", i + 1));
                    }
                    ...
                    if (mDtPayChecks.GetCellValue<string>("C_Deposit", i) == "Y")
                    {
                        if (mDtPayChecks.GetCellValue<string>("C_DpAcct", i) == "")
                        {
                            throw new Exception(String.Format("No se cuenta de deposito para cheque {0}", i + 1));
                        }
                        //lChecksToDeposit.Add(i);
                        lIntChekDeposit = i;
                    }
                    ...
                    AddInvoicesToPayment(ref lObjPayment, lDblCheckSum);
                    ...
                    else
                    {
                        ...
                        if (lIntChekDeposit >= 0)
                        {
                            CheckDTO lObjCheck = mCheckDAO.GetCheckByAttributes(
                                Convert.ToInt32(lStrLastDoc),
                                mDtPayChecks.GetCellValue<string>("C_AcctNum", lIntChekDeposit),
                                mDtPayChecks.GetCellValue<int>("C_CheckNum", lIntChekDeposit),
                                mDtPayChecks.GetCellValue<double>("C_CheckSum", lIntChekDeposit)
                            );
                            ...
                            lObjDeposit.DepositAccountType = SAPbobsCOM.BoDepositAccountTypeEnum.datBankAccount;
                            lObjDeposit.ReconcileAfterDeposit = SAPbobsCOM.BoYesNoEnum.tYES;
                            lObjDeposit.DepositAccount = lStrCahsAcct;
                            //lObjDeposit.DepositAccount = mDtPayChecks.GetCellValue<string>("C_DpAcct", lIntChekDeposit);
                            SAPbobsCOM.CheckLine lObjCheckLine = lObjDeposit.Checks.Add();
                            lObjCheckLine.CheckKey = lObjCheck.CheckKey;

                            SAPbobsCOM.DepositParams lObjDepositParams = lObjDepositService.AddDeposit(lObjDeposit);

                            AddDocumentsItem(lObjDepositParams.DepositNumber.ToString(),
                                             String.Format("Depósito de Cheque #{0}, de ${1:0.00}",
                                                           mDtPayChecks.GetCellValue<int>("C_CheckNum", lIntChekDeposit),
                                                           mDtPayChecks.GetCellValue<double>("C_CheckSum", lIntChekDeposit)),
                                             SAPbouiCOM.BoFormObjectEnum.fo_Deposit);
                ...
                ClearMatrix(mDtPayChecks, mMtxPayChecks);
                LoadChecks(mStrCardCode);
            }
            catch (Exception lObjException)
            {
                mBtnSave.Item.Enabled = true;
                LogUtility.WriteError(string.Format("[Payments - GenerateCheckDocuments] Error al crear los pagos para la sección de cheques: {0}", lObjException.Message));
        ...
        private void AddInvoicesToPayment(ref SAPbobsCOM.Payments pObjPayment, double pDblCheckSum)
        {
            try
            {
                int lIntDocLine = 0;
                if (!mChkDrafts.Checked)
                {
                    #region Invoices
                    // Add Invoice for every selected item in the matrix
                    for (int iInv = 0; iInv < mDtClientInvoices.Rows.Count; iInv++)
                    {
                        //double lDblDifference = mDtClientInvoices.GetCellValue<double>("C_Difference", iInv);
                        double lDblAmount = mDtClientInvoices.GetCellValue<double>("C_DocRem", iInv);

                        if (mDtClientInvoices.GetCellValue<string>("C_Selected", iInv) == "Y" && mDtClientInvoices.GetCellValue<double>("C_DocRem", iInv) > 0
                            && mDtClientInvoices.GetCellValue<string>("C_Used", iInv) == "N")
                        {
                            if (pDblCheckSum > 0)
                            {
                                double lDblSumApplied = mDtClientInvoices.GetCellValue<double>("C_DocRem", iInv);

                                pObjPayment.Invoices.DocLine = lIntDocLine++;
                                pObjPayment.Invoices.DocEntry = mDtClientInvoices.GetCellValue<int>("C_DocEntry", iInv);
                                pObjPayment.Invoices.InvoiceType = (SAPbobsCOM.BoRcptInvTypes)mDtClientInvoices.GetCellValue<int>("C_ObjType", iInv);
                                if (lDblSumApplied >= pDblCheckSum)
                                {
                                    pObjPayment.Invoices.SumApplied = pDblCheckSum;
                                    pDblCheckSum -= pDblCheckSum;
                                }
                                else
                                {
                                    pObjPayment.Invoices.SumApplied = lDblSumApplied;
                                    pDblCheckSum -= lDblSumApplied;
                                }

                                lDblAmount -= pObjPayment.Invoices.SumApplied;
                                mDtClientInvoices.SetValue("C_DocRem", iInv, lDblAmount);
                                if (lDblAmount <= 0)
                                {
                                    mDtClientInvoices.SetValue("C_Used", iInv, "Y");
                                }

                                pObjPayment.Invoices.Add();
                            }
                        }
                    }
                    #endregion
                ...
                {
                    #region Drafts
                    for (int iDraft = 0; iDraft < mDtClientInvoices.Rows.Count; iDraft++)
                    {
                        double lDblAmount = mDtClientInvoices.GetCellValue<double>("C_DocRem", iDraft);

                        if (mDtClientInvoices.GetCellValue<string>("C_Selected", iDraft) == "Y" && mDtClientInvoices.GetCellValue<double>("C_DocRem", iDraft) > 0
                            && mDtClientInvoices.GetCellValue<string>("C_Used", iDraft) == "N")
                        {
                            if (pDblCheckSum > 0)
                            {
                                SAPbobsCOM.Documents lDraft = (SAPbobsCOM.Documents)DIApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oDrafts);
                                lDraft.GetByKey(Convert.ToInt32(mDtClientInvoices.GetCellValue<double>("C_DocEntry", iDraft)));
                                //lDraft.EDocExportFormat = -96;
                                //lDraft.EDocGenerationType = SAPbobsCOM.EDocGenerationTypeEnum.edocGenerate;
                                lDraft.EDocGenerationType = SAPbobsCOM.EDocGenerationTypeEnum.edocNotRelevant;
                                //lDraft.EDocSeries = lDraft.Series;
                                ...
                                double lDblSumApplied = mDtClientInvoices.GetCellValue<double>("C_DocRem", iDraft);

                                pObjPayment.Invoices.DocLine = lIntDocLine++;
                                pObjPayment.Invoices.DocEntry = lDocEntry;
                                pObjPayment.Invoices.InvoiceType = (SAPbobsCOM.BoRcptInvTypes)mDtClientInvoices.GetCellValue<int>("C_ObjType", iDraft);
                                if (lDblSumApplied >= pDblCheckSum)
                                {
                                    pObjPayment.Invoices.SumApplied = lDblSumApplied;
                                    pDblCheckSum -= pDblCheckSum;
                                ...
                                lDblAmount -= pObjPayment.Invoices.SumApplied;
                                mDtClientInvoices.SetValue("C_DocRem", iDraft, lDblAmount);
                                if (lDblAmount <= 0)
                                {
                                    mDtClientInvoices.SetValue("C_Used", iDraft, "Y");
                                }
            ...
            {
                LogUtility.WriteError(string.Format("[Payments - AddInvoicesToPayment] Error al agregar las facturas al pago: {0}", lObjException.Message));
                throw new Exception(string.Format("Error al agregar las facturas al pago: {0}", lObjException.Message));
            }
        }

        private void AddDocumentsItem(string pStrDocEntry, string pStrComments = "Pago recibido", SAPbouiCOM.BoFormObjectEnum pEnmObjType = SAPbouiCOM.BoFormObjectEnum.fo_Receipt)
        {
            //mDtGeneratedDocuments.Rows.Clear();
            mDtGeneratedDocuments.Rows.Add();
            mDtGeneratedDocuments.SetValue("#", mDtGeneratedDocuments.Rows.Count - 1, mDtGeneratedDocuments.Rows.Count);
            mDtGeneratedDocuments.SetValue("C_DocEntry", 0, pStrDocEntry);
            mDtGeneratedDocuments.SetValue("C_Status", 0, "OK");
            mDtGeneratedDocuments.SetValue("C_Comments", 0, pStrComments);
            mDtGeneratedDocuments.SetValue("C_ObjType", 0, pEnmObjType);
        ...
        private void DoReconciliation()
        {
            try
            {
                //SAPbobsCOM.InternalReconciliationsService service2 = (SAPbobsCOM.InternalReconciliationsService)DIApplication.Company.GetCompanyService().GetBusinessService(SAPbobsCOM.ServiceTypes.InternalReconciliationsService);
                SAPbobsCOM.InternalReconciliationsService service = (SAPbobsCOM.InternalReconciliationsService)DIApplication.Company.GetCompanyService().GetBusinessService(SAPbobsCOM.ServiceTypes.InternalReconciliationsService);
                SAPbobsCOM.InternalReconciliationOpenTrans recon = (SAPbobsCOM.InternalReconciliationOpenTrans)service.GetDataInterface(SAPbobsCOM.InternalReconciliationsServiceDataInterfaces.irsInternalReconciliationOpenTrans);
                SAPbobsCOM.InternalReconciliationParams reconParams = (SAPbobsCOM.InternalReconciliationParams)service.GetDataInterface(SAPbobsCOM.InternalReconciliationsServiceDataInterfaces.irsInternalReconciliationParams);

                recon.CardOrAccount = SAPbobsCOM.CardOrAccountEnum.coaCard;
                recon.InternalReconciliationOpenTransRows.Add();
                recon.InternalReconciliationOpenTransRows.Item(0).Selected = SAPbobsCOM.BoYesNoEnum.tYES;
                recon.InternalReconciliationOpenTransRows.Item(0).TransId = 2213;
                recon.InternalReconciliationOpenTransRows.Item(0).ReconcileAmount = -0.01;
                recon.InternalReconciliationOpenTransRows.Add();
                recon.InternalReconciliationOpenTransRows.Item(0).Selected = SAPbobsCOM.BoYesNoEnum.tNO;
                recon.InternalReconciliationOpenTransRows.Item(0).TransId = 2222;
                recon.InternalReconciliationOpenTransRows.Item(0).ReconcileAmount = -0.01;

                reconParams = service.Add(recon);
                //recon.ReconType = SAPbobsCOM.ReconTypeEnum.rtManual;
            }
            catch (Exception lObjException)
            {
                LogUtility.WriteError(string.Format("[Payments - DoReconciliation] Error: {0}", lObjException.Message));
                throw lObjException;
            }
        }

        private void ClearMatrix(SAPbouiCOM.DataTable pStrDataTable, SAPbouiCOM.Matrix pObjMatrix)
        {
            if (!this.UIAPIRawForm.DataSources.DataTables.Item(pStrDataTable.UniqueID).IsEmpty)
            {
                this.UIAPIRawForm.DataSources.DataTables.Item(pStrDataTable.UniqueID).Rows.Clear();
                pObjMatrix.Clear();
            }
        }
